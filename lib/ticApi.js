...
const readChannel = async (channelRoot, iota = mamClient.getIota(), put = "put", del = "delete") => {
    const { messages } = await mamClient.getMessages(channelRoot, 'public', null, iota)
...
tic.contacts = {
    ...
    get: async (contactsRoot, iota = mamClient.getIota()) => {
        const { messages } = await mamClient.getMessages(contactsRoot, 'public', null, iota)
        const objects = messages.map(iota.utils.fromTrytes).map(JSON.parse)
        return messageBuilder.buildUniqueArray(objects, ["distrust", "trust"])
    }
}

const getChannelRoots = async (masterChannelRoot, iota = null) => {
    const response = await mamClient.getMessages(masterChannelRoot, 'public', null, iota)
